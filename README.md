...
Default model path in the app points to `checkpoints/local_lite/model.joblib`.
...
1. `cd` into `financial_distress/`, copy `.env.example` → **`.env`**, and set `OPENROUTER_API_KEY` there. **Do not put real keys only in `.env.example`** (that file is for templates). If `.env` is missing, the app can fall back to `.env.example` and will show a sidebar warning.
...
3. Run Streamlit from the **`financial_distress/`** folder so paths resolve. On **Lightning.ai / SSH**, create `.env` on the **remote** studio (or use secrets); your Mac `.env` is not visible there.
...
1. Place `data/ECL (1).csv` (or path aligned with `config.yaml`).
2. Run pipeline (network + SEC rate limits apply):
...
See `data/README.md` for artifact expectations.
